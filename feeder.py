    ...
    for dataset_idx, dataset in enumerate(tracks):
        ...
        for vehicle_idx, vehicle_data in enumerate(dataset):
            ...
            for frame_id, local_x, local_y, lane_id in vehicle_data.transpose():
                ...
                vehicle_dict[int(vehicle_id)] = np.array([dataset_id, vehicle_id, frame_id, local_x, local_y, lane_id],
                                                         dtype=np.float32)
...
class DataFeeder(Dataset):
    def __init__(self, mat_fpath, graph_args={}, t_h=30, t_f=50, down_sampling=2, neighbor_distance=90,
                 max_num_object=120, train_val_test='train', use_hetero_graph=False, **kwargs) -> None:
        ...
        self.future_frames = kwargs.get('t_f',
                                        t_f)  # length of predicted trajectory, 5 seconds with a sampling rate of 10 Hz
    ...
    def __getitem__(self, index):
        ...
        object_frame_feature = np.zeros(
            (self.max_num_object, self.total_frames // 2 + 1, 7))  # (V, T, C), T = 41 = 16 (history) + 25 (future)
    ...
    def _process_data(self, cur_frame_id, veh_id, dset_id, neighbor_grid, object_frame_feature):
        ...
        for i, v_id in enumerate(observed_veh_ids):
            ...
            object_frame_feature[valid_num, history_start: self.max_len_history,
            2: 6] = history  # [[frame_id, local_x, local_y, lane_id], ...]
            ...
            object_frame_feature[valid_num, self.max_len_history: future_end, -1] = 1 if len(
                history) > 0 or valid_num == 0 else 0  # (self.history_frames // self.down_sampling_steps // 2)
    ...
    def _get_tracks(self, veh_id, observed_frame_id, dset_id):
        ...
        track = self.tracks[dset_id - 1][
            veh_id - 1].transpose()  # (*, 4), the elements in second dimension are: frame_id, local_x, local_y, lane_id
        ...
        for i in range(visible_object_value.shape[0]):
            for j in range(visible_object_value.shape[0]):
                if neighbor_matrix[i][j] == 1:
                    local_y_i, lane_id_i = visible_object_value[i][
                                           4: 6]  # local_y and lane_id are the fifth and six-th feature in visible_object_value respectively.
        ...
        for i in range(visible_object_value.shape[0]):
            for j in range(0, i + 1):
                if neighbor_matrix[i][j] == 1:
                    ...
                    if lane_id_i > lane_id_j:
                        neighbor_type_matrix[i][j], neighbor_type_matrix[j][
                            i] = self.num_hetero_types - 1, self.num_hetero_types
                    elif lane_id_i < lane_id_j:
                        neighbor_type_matrix[i][j], neighbor_type_matrix[j][
                            i] = self.num_hetero_types, self.num_hetero_types - 1

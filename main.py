...
parser.add_argument('--num_epochs', default=1, type=int, help='')
...
def data_loader(*data_fpaths, batch_size=128, shuffle=True, drop_last=False, train_val_test='train'):
    ...
    for data_path in data_fpaths:
        dataset = DataFeeder(data_path, train_val_test=train_val_test, **args.__dict__)
    ...
    return loader, dataset.cumulative_sizes[-1] // batch_size + 1
    ...
    for i_epoch in range(trained_epochs, args.num_epochs):
        ...
        for iteration, (ori_data, neighbor_matrices, _, _) in enumerate(train_loader):
            ...
            for now_history_frames in [16]:
                ...
                predicted, _ = model.forward(input=input_data, soc_graph=A, pred_len=output_loc_GT.shape[-2], input_mask = input_mask, tfr = teacher_forcing, teacher = output_loc_GT)
...
if __name__ == '__main__':
    ...
    model = Seq2Seq(
        in_size=4, out_size=2, d_model=128, d_ffn=512,
